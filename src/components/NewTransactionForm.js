...
const newTransactionHandler = (event) => {
    ...
    const transaction = {
        key: `${newDate.current.value}${newCoin.current.value}`,
        date: newDate.current.value,
        coin: newCoin.current.value.trim(),
        qty: +newQty.current.value.trim(),
        costBasis: +newCostBasis.current.value.trim(),
        amountSpent: (newQty.current.value * newCostBasis.current.value).toFixed(2),
        exchange: newExchange.current.value.trim(),
        currentlyHeld: newCurrentlyHeld.current.value.trim()
    }
    ...
    return (
        <form onSubmit={newTransactionHandler}>
            ...
            <input type="number" id="costBasis" ref={newCostBasis} step='0.01'></input>
